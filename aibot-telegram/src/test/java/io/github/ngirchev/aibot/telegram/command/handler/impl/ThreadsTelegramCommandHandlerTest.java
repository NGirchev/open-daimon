...
class ThreadsTelegramCommandHandlerTest {
    ...
    void handleInner_whenMessageNull_thenThrows() {
        ...
        assertThrows(io.github.ngirchev.aibot.telegram.command.handler.TelegramCommandHandlerException.class,
                () -> handler.handleInner(command));
    }

    @Test
    void handleInner_whenNoThreads_thenReturnsNoConversationsMessage() throws TelegramApiException {
        Update update = new Update();
        Message message = new Message();
        message.setFrom(new User(200L, "user", false));
        update.setMessage(message);
