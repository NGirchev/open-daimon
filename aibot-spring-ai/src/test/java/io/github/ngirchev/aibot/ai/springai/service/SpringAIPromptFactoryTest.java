...
class SpringAIPromptFactoryTest {
    ...
    @Mock
    private io.github.ngirchev.aibot.ai.springai.tool.WebTools webTools;
    ...
    void preparePrompt_ollamaModel_callsChatModel() {
        var spec = promptFactory.preparePrompt(
                ...
                new AIBotChatOptions(0.7, 1000, null, "Hello", false, Map.of())
        );
        assertNotNull(spec);
    ...
    void preparePrompt_withBodyTemperatureAndMaxTokens_usesOverrides() {
        ...
        var spec = promptFactory.preparePrompt(
                ...
                new AIBotChatOptions(0.7, 1000, null, "Hi", false, Map.of())
        );
        assertNotNull(spec);
        ChatResponse response = spec.call().chatResponse();
        assertNotNull(response);
    ...
    void preparePrompt_withSystemAndUserMessages_includesBoth() {
        var spec = promptFactory.preparePrompt(
                ...
                new AIBotChatOptions(0.7, 1000, "You are helpful.", "Hello", false, Map.of())
        );
        assertNotNull(spec);
        ChatResponse response = spec.call().chatResponse();
        assertNotNull(response);
