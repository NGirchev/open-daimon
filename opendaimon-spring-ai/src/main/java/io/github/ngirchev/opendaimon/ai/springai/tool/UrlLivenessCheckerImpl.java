 ...
 * keyed by URL with a configurable TTL (typically minutes). The cache protects
 * the agent loop from repeated HTTP round-trips for the same URL within a single
 * session and amortizes cost across concurrent users asking overlapping questions.
 */
@Slf4j
