package ru.girchev.aibot.rest.repository;
...
import ru.girchev.aibot.rest.config.RestJpaConfig;
import ru.girchev.aibot.rest.config.RestFlywayConfig;
...
@TestPropertySource(properties = {
        "ai-bot.rest.enabled=true"
})
class RestAIBotMessageRepositoryIT {
    ...
    void whenSaveMessage_thenMessageIsSaved() {
        ...
        // Act
        AIBotMessage saved = messageRepository.save(message);
    ...
    void whenFindAll_thenReturnAllMessages() {
        ...
        AIBotMessage message2 = createRestMessage(user, "Second request", "127.0.0.1", "Mozilla/5.0", "/api/chat");
    ...
    void whenFindById_thenReturnMessage() {
        ...
        // Act
        AIBotMessage found = messageRepository.findById(saved.getId()).orElse(null);
    ...
    private AIBotMessage createUserMessage(RestUser user, ConversationThread thread, int sequenceNumber, String content) {
        AIBotMessage message = new AIBotMessage();
        message.setUser(user);
        message.setRole(MessageRole.USER);
    ...
    private AIBotMessage createAssistantMessage(RestUser user, ConversationThread thread,
    ...
    private AIBotMessage createRestMessage(RestUser user, String content, String clientIp, String userAgent, String endpoint) {
        AIBotMessage message = new AIBotMessage();
        message.setUser(user);
        message.setRole(MessageRole.USER);
        message.setContent(content);
