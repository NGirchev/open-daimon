...
/**
 * Manual E2E-like integration test for real Ollama + DOC document + follow-up RAG.
 *
 * <p>Unlike {@link ImagePdfVisionRagOllamaManualIT} which uses an image-only PDF requiring OCR
 * via a vision model, this test uses a regular DOC file ({@code file-sample_500kB.doc}) where
 * Apache Tika extracts text directly. No vision model is needed.
 *
 * <p>Flow:
 * <ol>
 *   <li>Send a message with {@code file-sample_500kB.doc} and prompt "что в первом предложении?"</li>
 *   <li>Tika extracts text, chunks are stored in RAG VectorStore</li>
 *   <li>Model answers using the extracted text</li>
 *   <li>Follow-up: "процитируй самое последнее предложение документа, которое заканчивается на 'sapien.'" —
 *       model uses RAG context</li>
 * </ol>
 *
...
@SpringBootTest(
        classes = DocRagOllamaManualIT.TestConfig.class,
...
class DocRagOllamaManualIT {
     ...
     * 1. Send a Telegram message with a DOC file and the prompt: "что в первом предложении?"
     * 2. Apache Tika extracts text directly (no vision/OCR needed).
     * 3. The extracted text is chunked and stored in RAG VectorStore.
     * 4. Ask a follow-up: "процитируй самое последнее предложение документа, которое заканчивается на 'sapien.'";
     *    RAG context is available to the model and it answers with the last sentence from the DOC.
    ...
    @Test
    @Timeout(6 * 60)
    @DisplayName("Manual E2E: DOC document + follow-up question uses stored RAG context")
    void doc_thenFollowUp_usesRagContext() throws IOException {
        Attachment docAttachment = loadDocAttachment();

        TelegramCommand firstCommand = createMessageCommand(
                TEST_CHAT_ID,
                1,
                "что в первом предложении?",
                List.of(docAttachment)
        );
        ...
        assertThat(extractedTextForRag.toLowerCase())
                .as("Extracted text must contain expected phrase from the DOC")
        ...
        TelegramCommand secondCommand = createMessageCommand(
                TEST_CHAT_ID,
                2,
                "процитируй самое последнее предложение документа, которое заканчивается на 'sapien.'",
                List.of()
        );
        ...
        assertThat(secondAssistantReply.toLowerCase())
                .withFailMessage(
                        "Follow-up answer should contain the last sentence from the DOC. Actual reply: [%s]",
    ...
    private Attachment loadDocAttachment() throws IOException {
        ClassPathResource resource = new ClassPathResource(DOC_RESOURCE);
        byte[] docBytes = resource.getInputStream().readAllBytes();
