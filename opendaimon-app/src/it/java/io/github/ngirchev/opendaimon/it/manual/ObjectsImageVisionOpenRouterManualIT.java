 ...
 *   -Dit.test=ObjectsImageVisionOpenRouterManualIT \
...
@SpringBootTest(
        classes = ObjectsImageVisionOpenRouterManualIT.TestConfig.class,
...
class ObjectsImageVisionOpenRouterManualIT {
     ...
     * 2. The image is sent directly to the vision model via OpenRouter (no RAG indexing for JPEG images).
    ...
    @Test
    @Timeout(3 * 60)
    @DisplayName("Manual E2E: OpenRouter auto + objects.jpeg — vision describes objects + follow-up")
    void objectsImage_openRouterVision_thenFollowUp() throws IOException {
