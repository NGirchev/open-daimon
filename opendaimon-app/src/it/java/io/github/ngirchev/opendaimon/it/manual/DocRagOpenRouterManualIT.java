 ...
 *
 * <p>Unlike image-only PDFs requiring OCR via a vision model, this test uses a regular DOC file
 * ({@code file-sample_500kB.doc}) where Apache Tika extracts text directly. No vision model
 * is needed.
 *
 * <p>Requires:
 * <ul>
 ...
 *   -Dit.test=DocRagOpenRouterManualIT \
...
@SpringBootTest(
        classes = DocRagOpenRouterManualIT.TestConfig.class,
...
class DocRagOpenRouterManualIT {
     ...
     * 5. Follow-up: "процитируй самое последнее предложение документа, которое заканчивается на 'sapien.'" —
     *    model uses RAG context and answers with the last sentence from the DOC.
    ...
    @Test
    @Timeout(3 * 60)
    @DisplayName("Manual E2E: OpenRouter auto + DOC document + follow-up RAG")
    void doc_openRouterAuto_thenFollowUp_usesRagContext() throws IOException {
    ...
    private TelegramCommand createMessageCommand(Long chatId, int messageId, String text, List<Attachment> attachments) {
        ...
        User from = new User();
        from.setId(chatId);
        from.setUserName("manual-openrouter-doc-user");
        from.setFirstName("Manual");
        from.setLastName("OpenRouterDoc");
