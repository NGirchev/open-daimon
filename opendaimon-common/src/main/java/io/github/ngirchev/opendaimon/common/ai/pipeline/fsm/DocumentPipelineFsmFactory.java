        ...
        var table = FsmFactory.INSTANCE.<AttachmentState, AttachmentEvent>statesWithEvents()
                ...
                .from(CLASSIFIED).toMultiple()
                    .to(IMAGE_PASSTHROUGH)
                        .condition(guard(AttachmentProcessingContext::isImage))
                        .end()
                    .to(ANALYZED)
                        .condition(guard(AttachmentProcessingContext::isDocument))
                ...
                .from(ANALYZED).toMultiple()
                    .to(TEXT_EXTRACTED)
                        .condition(guard(AttachmentProcessingContext::isTextExtractable))
                        .action(action(actions::extractText))
                        .end()
                    .to(VISION_OCR_COMPLETE)
                        .condition(guard(AttachmentProcessingContext::isImageOnly))
                ...
                .from(TEXT_EXTRACTED).toMultiple()
                    .to(RAG_INDEXED)
                        .condition(guard(AttachmentProcessingContext::hasExtractedChunks))
                ...
                .from(VISION_OCR_COMPLETE).toMultiple()
                    .to(RAG_INDEXED)
                        .condition(guard(AttachmentProcessingContext::isVisionOcrSucceeded))
                        .action(action(actions::indexInRag))
                        .end()
                    .to(IMAGE_FALLBACK)
                        .end()
